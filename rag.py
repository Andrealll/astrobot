...
sb = create_client(SUPABASE_URL, SUPABASE_KEY)import os
from typing import List, Dict, Any
from supabase import create_client

# fastembed import robusto (cambia tra versioni)
try:
    from fastembed import TextEmbedding
except ImportError:
...
def _get_model():
    global _MODEL
    if _MODEL is None:
        _MODEL = TextEmbedding(model_name=_EMBEDDING_MODEL)  # 384-d
    return _MODEL

def _embed_one(text: str) -> list:
    model = _get_model()
    return list(model.embed([text]))[0].astype("float32").tolist()  # (384,)
...
def _get_model():
    global _MODEL
    if _MODEL is None:
        _MODEL = TextEmbedding(model_name=_EMBEDDING_MODEL)
    return _MODEL
...
def search_topk(ctx: Dict[str, Any], top_k: int = 12):
    ...
    rows = res.data or []
    # Rerank leggero per metadati/priorità
    rows.sort(key=lambda r: _soft_match_score(r, ctx), reverse=True)
    return rows[:top_k]
